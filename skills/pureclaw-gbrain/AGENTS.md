...
6. **`gbrain doctor --fast`** — brain must be green before you rely on search or writes. Fix drift first.
...
- **Any message that names** a person, company, deal, meeting, or project — notability gate and filing rules are in **`SKILL.md`** §12 + the resolver docs (`RESOLVER.md` / `AGENTS.md` at the brain repo root).
- **Before web search or external APIs** — `gbrain search` / `gbrain query` first (**brain first**, `SKILL.md` §5).
- **After PureClaw Connect pulls data** — land structured facts in the brain with `gbrain put` per **`SKILL.md` §10** (connector → brain, not "mental notes" and not local files).

### How to use it

1. **Read `skills/pureclaw-gbrain/SKILL.md`** — Iron Laws, canonical `gbrain put` heredoc pattern, command cheat sheet (§7), dream cycle (§9), connector playbooks (§10).
